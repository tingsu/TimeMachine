...
def get_jacoco_coverage(restore_count):
    # adb shell am broadcast -a edu.gatech.m3.emma.COLLECT_COVERAGE
    cov_file_name = 'coverage' + str(restore_count)

    cmd = subprocess.Popen(['../../scripts/pull_coverage_jacoco.sh', cov_file_name],stdout = subprocess.PIPE, stderr = subprocess.PIPE)
    output, err = cmd.communicate()
...
def compute_coverage_jacoco():
    #using compute_coverage to compute
    cmd = subprocess.Popen ('../../scripts/compute_coverage_jacoco.sh',stdout = subprocess.PIPE, stderr = subprocess.PIPE)
    output, err=cmd.communicate()
    print 'compute_coverage-return code: ' + ' output: '+str(output) + '  ' + str(err) +' '
...
def pull_coverage_files(restore_count):
    if RunParameters.OPEN_SOURCE:
        ...
        get_jacoco_coverage(restore_count)
    else:
